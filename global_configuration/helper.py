...
from global_configuration.constants import S3_BUCKET, JWT_SECRET_KEY, JWT_AUDIENCE, API_CIRCLIN, INVALID_MIMES, \
    RESIZE_WIDTHS_IMAGE, RESIZE_WIDTHS_VIDEO, APP_ROOT, APP_TEMP, FFMPEG_PATH2, FFMPEG_PATH1
...
def video_to_mp4(path):
    ...
    subprocess.call([FFMPEG_PATH1, "-i", path, f"scale={width}x{height}", new_path])
    # if os.path.exists(path):
    #     os.remove(path)
